...
def search_with_bit_condition(
    input_list: List[str], bit_index: int = 0, least: bool = False
):
    if len(input_list) == 1:
        return input_list[0]
    ...
    if (most_common == 1 and not least) or (most_common == 0 and least):
        filtered_list = [
            binary
            for binary, leading_one in zip(input_list, leading_ones)
            if leading_one
        ]
    else:
        filtered_list = [
    ...
    return search_with_bit_condition(
        filtered_list, bit_index=bit_index + 1, least=least
    )
